...
CHO-264 landed alongside this change and adds two things the comparisons below
have to allow for: one trailing volatile message after all stored history (so
the replayed history is no longer the tail of the array), and a rolling
`cache_control` marker on the history tip. Both are additive request metadata,
which is exactly what CHO-264's conversation-store delta permits — "the
model-facing array may differ only by additive, non-semantic request metadata".
So byte-identity here is asserted modulo that marker, via
`_without_cache_control`, rather than by turning the cache flag off: that keeps
these tests running against the real shipped configuration.
"""
...
def _without_cache_control(obj):
    """Strip CHO-264's rolling cache marker so curation can be compared on its
    own terms. The marker is additive request metadata, never semantic content."""
    if isinstance(obj, dict):
        return {
            k: _without_cache_control(v)
            for k, v in obj.items()
            if k != "cache_control"
        }
    if isinstance(obj, list):
        return [_without_cache_control(v) for v in obj]
    return obj
...
def test_flag_on_sends_the_earlier_artifact_result_as_a_stub(app, monkeypatch):
    ...
    # CHO-264 appends one trailing volatile message (live context + reminders)
    # after all stored history, so the replayed history is no longer the tail.
    history = messages[-(len(lossless) + 1) : -1]
    ...
    assert json.dumps(_without_cache_control(history)) == json.dumps(
...
def test_flag_off_history_is_byte_identical_to_the_lossless_array(
    ...
    # after all stored history, so the replayed history is no longer the tail.
    history = messages[-(len(lossless) + 1) : -1]
    assert json.dumps(_without_cache_control(history)) == json.dumps(lossless)
