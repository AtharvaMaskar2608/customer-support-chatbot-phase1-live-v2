...
Each accordion panel header SHALL show a **22px rounded icon tile**, the segment title in uppercase, a muted count subline (`N rates`, singular `1 rate` when the segment has one line), and a chevron reflecting open/closed state. Tile colours SHALL use the FinX report four-colour family: Equity `#E8F0FE` / `#1D4FB8`, Derivative `#F0EBFE` / `#6941C6`, Commodity `#FEF4E6` / `#B76E00`, Currency `#E9F9F0` / `#17B26A`. Unknown segment titles SHALL use a neutral tile. Expanded panels SHALL list each line with the item title on the left and the rate display on the right.
